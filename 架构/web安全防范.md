...
```
## 例子2
直接构造链接：www.test.com/upvote?articleId=1024   
将链接丢在博客或者群聊，每个点击该链接的人都会为该文章点赞
## 防范
...
# JSON Hijacking
攻击者利用JSONP跨域传输数据的特性，诱导他人访问伪造好的请求或表单，获取敏感数据
## 例子
利用A网站比如微博，提供对外的JSONP接口，在加载危险网站时就会去请求A网站提供的JSONP数据接口，获取用户敏感数据（用户A网站cookie未过期的前提下。     
和CSRF攻击有所类似，但是在加载页面就可以获取数据，无需诱导用户进行其他操作。
## 防范
增加Refer校验，只有指定域内的请求才返回数据
# XSS跨站脚本攻击
## 定义
...
## 定义
攻击者将一台可用访问内网机器作为跳板，借此探测、攻击公司内网系统
## 举例
http://www.test.com/download?url=   
读取文件的接口，可以根据给定的URL读取图片文件   
当传入url=file:///etc/passwd
## 防范
- 给请求域设置白名单，只允许访问指定的域名
- 禁止30X跳转的请求（重定向）
- 禁止HTTP、HTTPS以外协议的请求
# Broken Access Control（垂直/水平越权攻击）
- 利用接口的未做权限控制，直接访问或者修改其他用户的数据
- 查看其他用户信息 - 水平越权
- 获取自己没有的权限 - 垂直越权
## 举例
用户首页有个查看个人资料的接口，供用户查看个人资料。但是链接形式是：   
www.test.com/userInfo?id=1001   
但是接口未做用户身份权限校验，当攻击者修改id为1002之类的，就可以看到全站用户的个人信息
## 防范
- 所有请求都要先对用户身份进行检查
- 权限校验的信息要从根据session等可信内容进行判断
# 参考资料
